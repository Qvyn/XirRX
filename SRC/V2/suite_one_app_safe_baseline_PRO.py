...
def _move_to_logs(filename: str):
    # Move APP_DIRilename to DIRS["logs"]ilename without suffix spam.
    try:
        ...
        try:
            # If file is empty and a dst exists, just remove the empty source.
            ...
            pass
        # Ensure logs dir
        try: os.makedirs(DIRS["logs"], exist_ok=True)
        except Exception: pass
        # Atomic replace (no numbered copies)
        try:
            os.replace(src, dst)
        except Exception:
...
# Ensure a default window icon exists so QSystemTrayIcon has one
        ...
        for b in (self.btn_input, self.btn_cross, self.btn_launch, self.btn_stream, self.btn_drift):
        ...
        self.streamerTab = None
        self.driftTab = DriftLabTab(self.stack)

        for w in (self.inputTab, self.crossTab, self.launcherTab, self.driftTab):
    ...
    def set_page(self, name: str):
        ...
        elif name == "Streamer" and self.streamerTab is not None:
            idx = self.stack.indexOf(self.streamerTab); self._check_btn(self.btn_stream)
        elif name == "Drift":
